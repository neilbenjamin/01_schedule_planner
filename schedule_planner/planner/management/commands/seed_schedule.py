...
from planner.models import Activation, Event, Performer, Venue
    ...
    def handle(self, *args, **kwargs):
        ...
        for (
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"Error parsing date {date_str}: {e}")
                )
                continue
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Error parsing time {start_str} or {end_str}: {e}"
                    )
                )
                continue
            ...
            self.stdout.write(
                f"Created event: {performer_name} at {venue_name} on {event_date}"
            )

        self.stdout.write(
            self.style.SUCCESS("Successfully seeded schedule from screenshot.")
        )
